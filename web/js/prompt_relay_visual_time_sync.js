...
const VISUAL_TIME_SYNC_NODE = "PromptRelayVisualTimeSync";
...
function safeLtxvTotalFrames(lengthSeconds, fps, safeDivisor) {
  const seconds = Number.isFinite(lengthSeconds) && lengthSeconds > 0 ? lengthSeconds : 30;
  const fpsValue = Number.isFinite(fps) && fps > 0 ? fps : 24;
  const divisor = Number.isFinite(safeDivisor) && safeDivisor > 0 ? Math.round(safeDivisor) : 8;
  return Math.ceil(seconds * fpsValue / divisor) * divisor + 1;
}

function distributeByLargestRemainder(lengths, targetTotal) {
  const n = lengths.length;
  if (n === 0) return [];
  const target = Math.max(n * MIN_SEGMENT_LENGTH, Math.round(targetTotal));
  const positive = lengths.map(v => Math.max(MIN_SEGMENT_LENGTH, parseInt(v, 10) || MIN_SEGMENT_LENGTH));
  const sourceTotal = positive.reduce((a, v) => a + v, 0);
  const exact = positive.map(v => v * target / sourceTotal);
  const result = exact.map(v => Math.max(MIN_SEGMENT_LENGTH, Math.floor(v)));

  while (result.reduce((a, v) => a + v, 0) > target) {
    let donor = -1;
    for (let i = 0; i < result.length; i++) {
      if (result[i] <= MIN_SEGMENT_LENGTH) continue;
      if (donor < 0 || result[i] > result[donor]) donor = i;
    }
    if (donor < 0) break;
    result[donor] -= 1;
  }

  let diff = target - result.reduce((a, v) => a + v, 0);
...
class TimelineEditor {
  constructor(node, container) {
    ...
    this.lengthSecondsWidget = node.widgets.find(w => w.name === "length_seconds");
    this.fpsWidget = node.widgets.find(w => w.name === "fps");
    this.safeDivisorWidget = node.widgets.find(w => w.name === "safe_divisor");
    ...
    if (this.isVisualTimeSync) this.rescaleTimelineToMax(this.getMaxFrames(), false);
  ...
  syncMaxFramesFromTime(rescale = true) {
    ...
    this.maxFramesWidget.value = nextMax;
    this._syncingMaxFrames = false;

    if (rescale && this.timeline?.segments?.length) {
      this.rescaleTimelineToMax(nextMax, true);
      this.updateUIFromSelection();
      this.render();
    }
    return true;
  ...
  bindEvents() {
    ...
    if (this.maxFramesWidget) {
      ...
      this.maxFramesWidget.callback = (...args) => {
        ...
        if (this.isVisualTimeSync && !this._syncingMaxFrames) {
          this.rescaleTimelineToMax(this.getMaxFrames(), false);
        } else {
          this.trimToFit();
        }
        this.commit();
    ...
    for (const w of [this.lengthSecondsWidget, this.safeDivisorWidget]) {
      if (!w) continue;
      const prev = w.callback;
      w.callback = (...args) => {
        prev?.apply(w, args);
        if (this.syncMaxFramesFromTime(true)) this.commit();
      };
    }
    if (this.fpsWidget) {
      ...
      this.fpsWidget.callback = (...args) => {
        ...
        if (this.isVisualTimeSync && this.syncMaxFramesFromTime(true)) this.commit();
        this.updateUIFromSelection();
        this.render();
      };
    }
    for (const w of [this.timeUnitsWidget]) {
      if (!w) continue;
      const prev = w.callback;
      w.callback = (...args) => {
        prev?.apply(w, args);
  ...
  rescaleTimelineToMax(maxFrames = this.getMaxFrames(), shouldCommit = true) {
    ...
    const target = Math.max(segs.length * MIN_SEGMENT_LENGTH, parseInt(maxFrames, 10) || 1);
    const current = segs.reduce((a, s) => a + (parseInt(s.length, 10) || MIN_SEGMENT_LENGTH), 0);
    if (current === target) {
      if (shouldCommit) this.commit();
      return;
    }
    const lengths = distributeByLargestRemainder(segs.map(s => s.length), target);
    for (let i = 0; i < segs.length; i++) segs[i].length = lengths[i];
    if (shouldCommit) this.commit();
  }
...
app.registerExtension({
  name: "PromptRelay.VisualTimeSync",

  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData?.name !== VISUAL_TIME_SYNC_NODE) return;
      ...
      this.onConfigure = function (info) {
        ...
        setTimeout(() => {
          if (this._timelineEditor) {
            this._timelineEditor.syncMaxFramesFromTime(false);
            this._timelineEditor.timeline = parseInitial(
              this._timelineEditor.timelineDataWidget?.value,
              this._timelineEditor.getMaxFrames(),
            );
            this._timelineEditor.rescaleTimelineToMax(this._timelineEditor.getMaxFrames(), false);
